...
export default [
  ...
  new EventMessage("You can almost afford a Bottle!", (state) => state.happiness > state.items[0]['bottle'].cost() / 2),
  new EventMessage("A Blankie! You would love that!", (state) => state.happiness >= state.items[0]['blankie'].cost() && state.items[0]['blankie'].owned === 0),
  new EventMessage("A Bottle! That will make you grow up faster!", (state) => state.happiness >= state.items[0]['bottle'].cost() && state.items[0]['bottle'].owned === 0),
